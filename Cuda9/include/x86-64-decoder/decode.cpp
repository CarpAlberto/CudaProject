...
resolve_gpr64(struct ud* u, enum ud_operand_code gpr_op)
{
  if (gpr_op >= OP_rAXr8 && gpr_op <= OP_rDIr15)
    gpr_op = (ud_operand_code)((gpr_op - OP_rAXr8) | (REX_B(u->pfx_rex) << 3));
  else 
	  gpr_op = (ud_operand_code)((gpr_op - OP_rAX));

  if (u->opr_mode == 16)
    return (ud_type)(gpr_op + UD_R_AX);
  if (u->dis_mode == 32 || 
    (u->opr_mode == 32 && ! (REX_W(u->pfx_rex) || u->default64))) {
    return (ud_type)(gpr_op + UD_R_EAX);
  }

  return (ud_type)(gpr_op + UD_R_RAX);
}
...
{
  gpr_op = (ud_operand_code)(gpr_op - OP_eAX);
...
{
  switch (type) {
    case T_MMX :    return (ud_type)(UD_R_MM0  + (i & 7));
    case T_XMM :    return (ud_type)(UD_R_XMM0 + i);
    case T_CRG :    return (ud_type)(UD_R_CR0  + i);
    case T_DBG :    return (ud_type)(UD_R_DR0  + i);
    case T_SEG :    return (ud_type)(UD_R_ES   + (i & 7));
    case T_NONE:
...
static int disasm_operands(register struct ud* u)
{
  ...
  switch(mop1t) {
    ...
    case OP_AH : case OP_CH : case OP_DH : case OP_BH :
        ...
        iop[0].base = (ud_type)(UD_R_AL + (mop1t - OP_AL));
        iop[0].size = 8;
    ...
    case OP_SS : case OP_FS : case OP_GS :
        ...
        if (u->dis_mode == 64)
            if (mop1t != OP_FS && mop1t != OP_GS)
                u->error= 1;
        iop[0].type = UD_OP_REG;
        iop[0].base = (ud_type)((mop1t - OP_ES) + UD_R_ES);
        iop[0].size = 16;
    ...
    case OP_ST4 : case OP_ST5 : case OP_ST6 : case OP_ST7 :
        ...
        iop[0].type = UD_OP_REG;
        iop[0].base = (ud_type)((mop1t-OP_ST0) + UD_R_ST0);
        iop[0].size = 0;

        if (mop2t >= OP_ST0 && mop2t <= OP_ST7) {
            iop[1].type = UD_OP_REG;
            iop[1].base = (ud_type)((mop2t-OP_ST0) + UD_R_ST0);
            iop[1].size = 0;
        }
        break;
